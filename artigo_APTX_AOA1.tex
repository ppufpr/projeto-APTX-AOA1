...
\newline
\\
Pedro Henrique Ramos Prado\textsuperscript{1*}
\\
\bigskip
...
\emph{desligado} nas células sem aprataxina. Para chegar a essa conclusão,
aplicamos métodos de Biologia de Sistemas: transformamos a expressão dos genes em
...
destacam os métodos de Biologia de Sistemas (discretização, modelagem booleana e
...
\paragraph{Discretização e modelagem booleana.} Para aplicar os métodos de
Biologia de Sistemas, definiu-se um painel de 30 genes composto pelo módulo das
...
\emph{quantizados} em duas representações discretas, conforme apresentado na
disciplina: (i) \emph{quantização binária}, atribuindo \(1\) (superexpresso)
...
do painel foi inferida por dois critérios de relevância ensinados na disciplina:
...
o contraste. Recomenda-se, portanto, descrever o contraste principal como
...
Como método central da disciplina, a rede de co-regulação do painel foi inferida
...
\gene{ATM} e \gene{PARP1} apresentaram os maiores valores de grau e
intermediação, destacando-se como os nós mais conectados do módulo
(Tabela~\ref{tab:string}). O \gene{APTX}, embora com grau relativamente menor,
...
Recomenda-se, portanto, descrever o achado como ``perda de função de \gene{APTX}''.
...
a direção observada --- ISGs reprimidos no KO --- é concordante com o estudo de
...
A aplicação dos métodos de Biologia de Sistemas --- discretização, modelagem
...
robustas. Trata-se de uma limitação inerente ao tamanho amostral (\(n=12\)), e
não do arcabouço de informação mútua em si.
...
ataxias de reparo de DNA. Em conjunto --- e reforçados por uma reanálise
independente que reproduziu os resultados centrais ---, os achados são
...
(iii) aplicou métodos de Biologia de Sistemas --- discretização, modelagem
...
justifica pela maior robustez de estimadores lineares sob amostragem limitada ---
uma propriedade metodológica diretamente ligada ao desempenho observado. O
...
\textbf{Ferramenta utilizada.} Assistente de IA generativa (modelo de linguagem
de grande porte com acesso a ambiente computacional).

\textbf{Como e onde foi utilizada.} A ferramenta foi empregada como apoio em
quatro etapas: (i) \emph{revisão crítica} do manuscrito original e da metodologia;
(ii) \emph{reanálise} dos dados públicos (expressão diferencial com DESeq2,
enriquecimento funcional, discretização, modelagem booleana, inferência de rede
regulatória e validação por métricas); (iii) \emph{geração das figuras}; e
(iv) \emph{redação e formatação} em \LaTeX{}. Todos os resultados numéricos foram
computados a partir dos dados reais (GSE245766), verificados por reexecução dos
\emph{scripts}, e a interpretação científica e as decisões metodológicas são de
...
\begin{enumerate}
\item ``Entender o artigo e fazer uma revisão crítica do manuscrito.''
\item ``Fazer ambas as recomendações da revisão'' (reanálise: contraste de
genótipo sob o modelo com interação; teste de permutação do AUC; escore de
assinatura de interferon).
\item ``Criar um documento padrão \LaTeX{}, em pt-BR, com um artigo baseado no
relatório e nos novos resultados.''
\item ``Adicionar ao artigo gráficos e dados obtidos com os métodos aprendidos na
disciplina'' (discretização, redes booleanas, inferência de GRN por correlação e
informação mútua, validação contra STRING com AUC/precisão/\emph{recall}/F1).
\item ``Verificar, com base na solicitação da professora, se o trabalho está
adequado'' e ``adaptar o artigo às normas da revista \emph{PLOS Computational
Biology}.''
\end{enumerate}
